...
namespace MonoGameWindowsStarter
{
    public class SpriteSheet
    {
        ...
        public SpriteSheet(Texture2D texture, int width, int height, int offset = 0, int gutter = 0, int ver_gutter = 0)
        {
            sheet = texture;
            var columns = (texture.Width - offset) / (width + gutter);
            var rows = (texture.Height - offset) / (height + ver_gutter);
